...
import com.yanweiyi.micodebackend.mapper.UserTagMapper;
...
public class UserTagServiceImpl extends ServiceImpl<UserTagMapper, UserTag> implements UserTagService {
    ...
    public Map<String, List<String>> findStructuredTags(Object...obj) {
        List<UserTag> userTagList = this.list();

        // 首先，创建一个映射 parentId: tagName 的父标签 Map
        Map<Long, String> parentTags = userTagList.stream()
                .filter(UserTag::isParentBool) // 使用 isParentBool() 方法过滤出父标签
                .collect(Collectors.toMap(UserTag::getId, UserTag::getTagName));
